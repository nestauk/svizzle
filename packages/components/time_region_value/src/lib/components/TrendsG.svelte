<script>
	...
	$: trends = makeTrends(data.filteredData);
	$: taggedTrends = _.map(trends, item => ({
		...item,
		preselected: _.has(data.preselectedRegions, item.key),
		selected: _.has(data.selectedRegions, item.key),
	}));
	...
	$: xMed = (x1 + x2) / 2;
	$: radius = Math.min($_layout.radius, 0.5 * (height / maxOrder) - gap);
	...
	$: quadTree = isIterableNotEmpty(data.filteredData) &&
		quadtree()
		.x(getX)
		.y(getY)
		.addAll(data.filteredData)
		...
		<g class='curves'>
			{#each trendLines as {key, value, preselected, selected}}
				<path
					...
					class:dimmed='{$_tooltip.isVisible && highlightedKey !== key}'
					class:focused='{$_tooltip.isVisible && highlightedKey === key}'
					d={value}
					stroke='url(#{key})'
				/>
			{/each}
		...
		{#if $_tooltip.isVisible}
			...
			>
				...
				>
					...
					<text dy={tooltipShift} class='bkg'>{$_tooltip.nuts_label}</text>
					<text dy={tooltipShift}>{$_tooltip.nuts_label}</text>
				</g>
			</g>
		{/if}
