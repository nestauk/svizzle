...
import {
	...
	_regionSettings,
} from 'stores/regionSettings';
import {getLevel, getName, getRootId} from 'utils/domain';
...
const getRootIds = _.pipe([_.getKey(0), _.keys]);
const rootIds = getRootIds(initialSelection);
...
const getRoots = _.pipe([_.getKey(0), getLevelRegions]);
const makeFirstRootId = _.pipe([
	getRoots,
	_.sortWith([getName]),
	_.getPath('0.id')
]);
...
export const _roots = derived(_regionsSelection, getRoots);
...
export const _focusedRootChildren = derived(
	[_currentLevel, _focusedRootId, _regionsSelection],
	([currentLevel, focusedRootId, regionsSelection]) =>
		_.values(regionsSelection[currentLevel][focusedRootId])
);
