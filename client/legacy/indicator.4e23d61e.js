import { cH as mapValues, bR as sortWith, cI as sorter, a0 as getKey, V as transformValues, ah as getPath, be as writable, cx as derived, cJ as getAt, cb as get_store_value, Z as pipe, T as values, cK as groupBy, _ as mapValuesWith, cv as indexBy, cL as setKey, cM as flatMapWith, cN as isGT, R as keys, c2 as map, c4 as isNotNil, bu as uniques, bG as updateKey, cO as updateKeys, cP as tail, bo as mergeObj, cQ as skip, K as makeStyleVars, cC as filter, c3 as reduce, cf as sorterDesc, c7 as flatten } from './client.9324a931.js';
import { b as rgbBasis } from './rgb.1861a087.js';
import { p as _lookup, q as makeGetIndicatorFormatOf, k as _selectedYear } from './stores.a1f53498.js';
import { h as getId } from './defaultLocale.a39c0f65.js';
import { d as setIndexAsKey } from './linear.f307c541.js';
...
const sanitizeDatapoint = id => transformValues({
  [id]: Number,
  region_level: Number,
  region_year_spec: Number,
  year: Number
});
const parseCSV = id => datapoint => csvParse(datapoint, sanitizeDatapoint(id));
...
let initialLevel = ((_get = get_store_value(_regionSettings)) === null || _get === void 0 ? void 0 : _get.initialLevel) || 0;
const _currentLevel = writable(initialLevel);
_regionSettings.subscribe(_ref => {
  let {
...
const makeInitialSelection = pipe([values, groupBy(getLevel), mapValuesWith(pipe([groupBy(getRootId), mapValuesWith(pipe([indexBy(getId), mapValuesWith(setKey('status', 1))]))]))]);
const initialSelection = makeInitialSelection(hierarchy);
const _regionsSelection = writable(initialSelection);

/* utils */

const sortByName = sortWith([getName]);
const getValuesSortedByName = pipe([values, sortByName]);
...
const getLevelRegions = pipe([values, flatMapWith(values)]);

/* menu */

// focused root id

const getRootsSortedByName = pipe([getKey(0), getLevelRegions, sortByName]);
const makeFirstRootId = pipe([getRootsSortedByName, getPath('0.id')]);
...
const _focusedRoot = derived([_focusedRootId, _regionsSelection], _ref2 => {
  let [focusedRootId, regionsSelection] = _ref2;
...
const _roots = derived(_regionsSelection, getRootsSortedByName);

// update all roots

const updateAllRootsWith = updater => () => _regionsSelection.update(mapValuesWith(mapValuesWith(mapValuesWith(updater))));
...
const toggleRoot = rootId => _regionsSelection.update(selection => {
  const rootStatus = selection[0][rootId][rootId].status;
  const newStatus = rootStatus !== 1 ? 1 : 0;
  const newSelection = mapValues(selection, updateKey(rootId, mapValuesWith(makeSetStatusTo(newStatus))));
  return newSelection;
});
...
const _focusedRootDescendants = derived([_currentLevel, _focusedRootId, _regionsSelection], _ref3 => {
  let [currentLevel, focusedRootId, regionsSelection] = _ref3;
...
const selectAllFocusedRootChildren = updateAllFocusedRootChildrenWith(setStatusTo1);
const deselectAllFocusedRootChildren = updateAllFocusedRootChildrenWith(setStatusTo0);
...
const toggleFocusedRootDescendant = regionId => {
  ...
  while (regions.length > 0) {
    ...
    if (isNotNil(pid)) {
      const siblingsStatuses = uniques(map(hierarchy[pid].children, id => selection[level][focusedRootId][id].status));
...
const _isRegionsSelectionDirty = derived(_regionsSelection, regionsSelection => {
  const currentLevel = get_store_value(_currentLevel);
...
const setCurrentLevel = newLevel => {
  ...
  if (didSelectLowerLevel) {
    ...
    let regions = map(getLevelRegions(selection[newLevel]), item => ({
      item
    ...
    while (parentLevel >= firstAvailableLevel) {
      ...
      while (regions_.length > 0) {
        ...
        if (isNotNil(children)) {
          const childrenStatuses = uniques(map(children, cid => selection[level + 1][rootId][cid].status));
...
const _style = derived(_theme, pipe([skip(['colorSchemes']), makeStyleVars]));
...
const _reconciledIndicator = derived(_indicator, _ref => {
  let {
    data,
    id
  } = _ref;
  const reconciledData = map(data, datapoint => {
    ...
    const yearlyNutsId = "".concat(region_year_spec, "/").concat(region_id);
...
const _availableYears = derived([_indicator, _lookup], _ref2 => {
  let [{
    id
  }, lookup] = _ref2;
...
const _formatFn = derived([_indicator, _lookup], _ref3 => {
  let [{
    id
  }, lookup] = _ref3;
...
const _getIndicatorValue = derived(_indicator, _ref4 => {
  let {
    id
  } = _ref4;
  return getKey(id);
});
const makeSetOrderWith = accessor => pipe([groupBy(getYear), mapValuesWith(pipe([sortWith([sorterDesc(accessor)]), setIndexAsKey('order')])), values, flatten]);
...
const _levelData = derived([_currentLevel, _reconciledIndicator], _ref5 => {
  let [currentLevel, {
    id,
    data
  }] = _ref5;
  return id && filter(data, _ref6 => {
    let {
      region_level
    } = _ref6;
...
const _rankedData = derived([_levelData, _setOrder], _ref7 => {
  let [levelData, setOrder] = _ref7;
  return setOrder(levelData);
});
const _selectionData = derived([_doFilterRegions, _rankedData, _regionsSelection], _ref8 => {
  let [doFilterRegions, rankedData, selection] = _ref8;
  return reduce(rankedData, (acc, datapoint) => {
    const {
      region: {
        id,
        level,
...
const _yearSelectionData = derived([_selectedYear, _selectionData], _ref9 => {
  let [selectedYear, selectionData] = _ref9;
  return selectedYear && filter(selectionData, _ref10 => {
    let {
      year
    } = _ref10;
...
const _regionsYearSpec = derived(_yearSelectionData, yearSelectionData => {
...
const _selectedRegionAtlasIds = derived(_yearSelectionData, yearSelectionData => reduce(yearSelectionData, (acc, datapoint) => {
...
*/
const _selectedRegionIds = derived(_yearSelectionData, yearSelectionData => reduce(yearSelectionData, (acc, datapoint) => {
  if (datapoint.isSelected) {
