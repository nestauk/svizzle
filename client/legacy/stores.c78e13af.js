import { ae as partial, af as __, cB as apply, W as pipe, a6 as collect, cC as getLength, au as reduceWith, cD as sum, cE as pluck, cp as flatten, cF as indexBy, as as getPath, cG as transpose, cH as derived, by as writable, aM as _screen, cm as adapter, cI as casus, cJ as isNil, cj as allOf, cv as isKeyValue, co as hasKey, ch as not, bE as identity, cK as getTruthyValuesKeys, cL as joinWithBlank, a4 as mapValuesWith, bD as readable } from './client.c3033a77.js';
import { s as sliceString, c as arraySumWith, l as linear } from './linear.1b91202b.js';
import { g as getValue, m as makeArrayTransformer, i as inclusiveRange, c as format } from './defaultLocale.cc2e245b.js';
...
const _glyph = derived(_screen, s => s?.glyph);
...
const makeGetIndicatorFormatOf = id => pipe([getPath(`${id}.schema.value`), adapter([casus(isNil, getIdentity), casus(allOf([isKeyValue(['data_type', 'int']), hasKey('format')]), value => pipe([Math.round, format(value.format)])), casus(allOf([isKeyValue(['data_type', 'int']), not(hasKey('format'))]), () => Math.round), casus(hasKey('format'), value => format(value.format)), getIdentity])]);
const makeGetRefFormatOf = id => pipe([getPath(`${id}.schema.value`), adapter([casus(isNil, getIdentity), casus(allOf([isKeyValue(['data_type', 'int']), hasKey('format')]), value => pipe([Math.round, format(value.format)])), casus(isKeyValue(['data_type', 'int']), () => Math.round), casus(hasKey('format'), value => format(value.format)), getIdentity])]);
