import { a4 as partial, a5 as __, bQ as make, E as _slicedToArray, bR as tail, bJ as derived, a0 as pipe, bS as skip, ag as makeStyleVars, bG as pluck, b2 as _toConsumableArray, be as map, a6 as zip, am as writable, ao as mergeObj, $ as getKey, bT as getAt, bU as mapValues, b0 as sortWith, bV as sorter, X as transformValues, R as _defineProperty, b6 as getPath, bW as get_store_value, bX as values, bY as groupBy, a1 as mapValuesWith, bH as indexBy, bZ as setKey, V as keys, b_ as flatMapWith, b$ as isGT, bg as isNotNil, au as uniques, aT as updateKey, c0 as updateKeys, c1 as filter, bf as reduce, c2 as isIterableEmpty, bu as sorterDesc, bk as flatten } from './client.50a99e71.js';
import { p as _lookup, q as makeGetIndicatorFormatOf, k as _selectedYear } from './stores.8e11225e.js';
import { d as getId, e as setIndexAsKey } from './yootils.es.ac6ee4e8.js';
import { i as initRange, b as linearish, e as bisect, r as rgbBasis } from './linear.0c4bf1cb.js';
import { p as pairs, e as extent } from './Info.9508c99c.js';
...
var _makeColorScale = derived(_baseColorScale, function (_baseColorScale) {
  return function (extent) {
    return _baseColorScale.domain(extent);
  };
});
var _makeColorBins = derived([_baseColorScale, _colorScheme], function (_ref4) {
  ...
  return function (colScale) {
    ...
    var ranges = pairs([domain[0]].concat(_toConsumableArray(baseColorScale.thresholds()), [domain[1]]));
...
var _availableLevels = derived(_regionSettings, getKey('levels'));
var _firstAvailableLevel = derived(_availableLevels, getAt(0));
...
var getYear = getKey('year'); // regions

var getLevel = getKey('level');
var getName = getKey('name');
var getRootId = getKey('rootId');
var getRegionAtlasId = getPath('region.id');
/* region labels */

var regionIdToName = mapValues(hierarchy, getName);
/* sorting */

var sortAscByYear = sortWith([sorter(getYear)]);
/* rest */

var sanitizeValue = function sanitizeValue(id) {
  var _transformValues;

  return transformValues((_transformValues = {}, _defineProperty(_transformValues, id, Number), _defineProperty(_transformValues, "region_level", Number), _defineProperty(_transformValues, "region_year_spec", Number), _defineProperty(_transformValues, "year", Number), _transformValues));
};
...
var _doFilterRegions = writable(false); // level
...
var initialLevel = ((_get = get_store_value(_regionSettings)) === null || _get === void 0 ? void 0 : _get.initialLevel) || 0;
var _currentLevel = writable(initialLevel);

_regionSettings.subscribe(function (_ref) {
  var initialLevel = _ref.initialLevel;
  initialLevel && _currentLevel.set(initialLevel);
});
/* selected regions */


var makeInitialSelection = pipe([values, groupBy(getLevel), mapValuesWith(pipe([groupBy(getRootId), mapValuesWith(pipe([indexBy(getId), mapValuesWith(setKey('status', 1))]))]))]);

var initialSelection = makeInitialSelection(hierarchy);
var _regionsSelection = writable(initialSelection); // roots ids

var getRootIds = pipe([getKey(0), keys]);

getRootIds(initialSelection);
/* utils */
// updaters

var makeSetStatusTo = function makeSetStatusTo(value) {
  return setKey('status', value);
};

var setStatusTo0 = makeSetStatusTo(0);
var setStatusTo1 = makeSetStatusTo(1); // levels

var getLevelRegions = pipe([values, flatMapWith(values)]);
/* menu */
// focused root id


var getRoots = pipe([getKey(0), getLevelRegions]);

var makeFirstRootId = pipe([getRoots, sortWith([getName]), getPath('0.id')]);

var firstFocusedRootId = makeFirstRootId(initialSelection);
var _focusedRootId = writable(firstFocusedRootId);
var focusRoot = function focusRoot(rootId) {
  return _focusedRootId.set(rootId);
}; // focused root

var _focusedRoot = derived([_focusedRootId, _regionsSelection], function (_ref2) {
  var _ref3 = _slicedToArray(_ref2, 2),
      focusedRootId = _ref3[0],
      regionsSelection = _ref3[1];

  return regionsSelection[0][focusedRootId][focusedRootId];
}); // layout

var _isMenuExpanded = derived(_currentLevel, isGT(0));
/* menu: roots */
// roots

var _roots = derived(_regionsSelection, getRoots); // update all roots

var updateAllRootsWith = function updateAllRootsWith(updater) {
  return function () {
    return _regionsSelection.update(mapValuesWith(mapValuesWith(mapValuesWith(updater))));
  };
};
var selectAllRoots = updateAllRootsWith(setStatusTo1);
var deselectAllRoots = updateAllRootsWith(setStatusTo0); // update a root and all its descendents

var toggleRoot = function toggleRoot(rootId) {
  return _regionsSelection.update(function (selection) {
    var rootStatus = selection[0][rootId][rootId].status;
    var newStatus = rootStatus !== 1 ? 1 : 0;

    var newSelection = mapValues(selection, updateKey(rootId, mapValuesWith(makeSetStatusTo(newStatus))));
...
var _focusedRootChildren = derived([_currentLevel, _focusedRootId, _regionsSelection], function (_ref4) {
  var _ref5 = _slicedToArray(_ref4, 3),
      currentLevel = _ref5[0],
      focusedRootId = _ref5[1],
      regionsSelection = _ref5[2];

  return values(regionsSelection[currentLevel][focusedRootId]);
}); // update all descendants of the focused root at the current level

var updateAllFocusedRootChildrenWith = function updateAllFocusedRootChildrenWith(statusUpdater) {
  return function () {
    var availableLevels = get_store_value(_availableLevels);
    var focusedRootId = get_store_value(_focusedRootId); // e.g. selecting all descendants of FR at level 2
    // should select FR and all of the regions in that branch

    var updateFn = updateKeys({
      keys: availableLevels,
      updater: updateKey(focusedRootId, mapValuesWith(statusUpdater))
    });

    _regionsSelection.update(updateFn);
  };
};
var selectAllFocusedRootChildren = updateAllFocusedRootChildrenWith(setStatusTo1);
var deselectAllFocusedRootChildren = updateAllFocusedRootChildrenWith(setStatusTo0); // update a specific child of the focused root at the current level

var toggleFocusedRootDescendant = function toggleFocusedRootDescendant(regionId) {
  var currentLevel = get_store_value(_currentLevel);
  var firstAvailableLevel = get_store_value(_firstAvailableLevel);
  var focusedRootId = get_store_value(_focusedRootId);
  var selection = get_store_value(_regionsSelection);
  var region = selection[currentLevel][focusedRootId][regionId];
  var newStatus = region.status !== 1 ? 1 : 0; // region & descendants:
  // descendants take same status as toggled region

  var regions = [region];

  while (regions.length > 0) {
    var _regions$shift = regions.shift(),
        children = _regions$shift.children,
        id = _regions$shift.id,
        level = _regions$shift.level;

    selection[level][focusedRootId][id].status = newStatus;

    if (children) {
      regions = regions.concat(map(children, function (cid) {
  ...
  var _loop = function _loop() {
    var _regions$shift2 = regions.shift(),
        pid = _regions$shift2.pid,
        level = _regions$shift2.level;

    if (isNotNil(pid)) {
      var siblingsStatuses = uniques(map(hierarchy[pid].children, function (id) {
        return selection[level][focusedRootId][id].status;
      }));

      var parentLevel = level - 1;

      if (parentLevel >= firstAvailableLevel) {
        selection[parentLevel][focusedRootId][pid].status = siblingsStatuses.length === 1 ? siblingsStatuses[0] : -1;
      }
...
var _isRegionsSelectionDirty = derived(_regionsSelection, function (regionsSelection) {
  var currentLevel = get_store_value(_currentLevel);
  var currentLevelSelection = regionsSelection[currentLevel];
  var isDirty = false;

  var rootIds = keys(currentLevelSelection);

  while (rootIds.length > 0) {
    var rootId = rootIds.shift();

    var ids = keys(currentLevelSelection[rootId]);
...
var setCurrentLevel = function setCurrentLevel(newLevel) {
  var previousLevel = get_store_value(_currentLevel);
  var didSelectLowerLevel = newLevel < previousLevel;

  if (didSelectLowerLevel) {
    (function () {
      var selection = get_store_value(_regionsSelection); // region & descendants:
      // for all regions at new level:
      // 	if region is partial:
      // 		- set status to 1
      // 		- set all descendants to 1

      var regions = map(getLevelRegions(selection[newLevel]), function (item) {
        return {
          item: item
        };
      } // newLevel regions, don't pass a `status`
      );

      var _loop2 = function _loop2() {
        var _regions$shift4 = regions.shift(),
            item = _regions$shift4.item,
            status = _regions$shift4.status;

        var children = item.children,
            id = item.id,
            level = item.level,
            rootId = item.rootId;
        var region = selection[level][rootId][id]; // get the new status
        ...
        if (newStatus) {
          ...
          if (children) {
            regions = regions.concat(map(children, function (cid) {
              var _hierarchy$cid = hierarchy[cid],
                  id = _hierarchy$cid.id,
                  level = _hierarchy$cid.level,
                  rootId = _hierarchy$cid.rootId;
              return {
      ...
      while (parentLevel >= firstAvailableLevel) {
        var _regions = getLevelRegions(selection[parentLevel]);

        var _loop3 = function _loop3() {
          var _regions$shift3 = _regions.shift(),
              children = _regions$shift3.children,
              level = _regions$shift3.level,
              rootId = _regions$shift3.rootId,
              id = _regions$shift3.id;

          if (isNotNil(children)) {
            var childrenStatuses = uniques(map(children, function (cid) {
              return selection[level + 1][rootId][cid].status;
            }));

            selection[level][rootId][id].status = childrenStatuses.length === 1 ? childrenStatuses[0] : -1;
          }
        };

        while (_regions.length > 0) {
          _loop3();
        }
...
var _indicator = writable({
  data: [],
  id: undefined
});
var _reconciledIndicator = derived(_indicator, function (_ref) {
  var data = _ref.data,
      id = _ref.id;

  var reconciledData = map(data, function (datapoint) {
    var region_id = datapoint.region_id,
        region_year_spec = datapoint.region_year_spec;
    var yearlyNutsId = "".concat(region_year_spec, "/").concat(region_id);
    var atlasId = yearlyNutsIdToId[yearlyNutsId];
    var region = hierarchy[atlasId];
...
var _availableYears = derived([_indicator, _lookup], function (_ref2) {
  var _ref3 = _slicedToArray(_ref2, 2),
      id = _ref3[0].id,
      lookup = _ref3[1];

  return id ? lookup[id].availableYears : [];
});
/* current indicator utils */

var _formatFn = derived([_indicator, _lookup], function (_ref4) {
  var _ref5 = _slicedToArray(_ref4, 2),
      id = _ref5[0].id,
      lookup = _ref5[1];

  var getIndicatorFormat = makeGetIndicatorFormatOf(id);
  var formatFn = getIndicatorFormat(lookup);
  return formatFn;
});
var _getIndicatorValue = derived(_indicator, function (_ref6) {
  var id = _ref6.id;
  return getKey(id);
});

var makeSetOrderWith = function makeSetOrderWith(accessor) {
  return pipe([groupBy(getYear), mapValuesWith(pipe([sortWith([sorterDesc(accessor)]), setIndexAsKey('order')])), values, flatten]);
};

var _setOrder = derived(_getIndicatorValue, makeSetOrderWith);
/* data */


var _levelData = derived([_currentLevel, _reconciledIndicator], function (_ref7) {
  var _ref8 = _slicedToArray(_ref7, 2),
      currentLevel = _ref8[0],
      _ref8$ = _ref8[1],
      id = _ref8$.id,
      data = _ref8$.data;

  return id && filter(data, function (_ref9) {
    var region_level = _ref9.region_level;
    return region_level === currentLevel;
  }) || [];
});
var _rankedData = derived([_levelData, _setOrder], function (_ref10) {
  var _ref11 = _slicedToArray(_ref10, 2),
      levelData = _ref11[0],
      setOrder = _ref11[1];

  return setOrder(levelData);
});
var _selectedData = derived([_doFilterRegions, _rankedData, _regionsSelection], function (_ref12) {
  var _ref13 = _slicedToArray(_ref12, 3),
      doFilterRegions = _ref13[0],
      rankedData = _ref13[1],
      selection = _ref13[2];

  return reduce(rankedData, function (acc, datapoint) {
    var _datapoint$region = datapoint.region,
        id = _datapoint$region.id,
        level = _datapoint$region.level,
        rootId = _datapoint$region.rootId;
    var isSelected = selection[level][rootId][id].status === 1;

    if (!doFilterRegions || isSelected) {
      acc.push(_objectSpread(_objectSpread({}, datapoint), {}, {
...
var _noData = derived(_selectedData, isIterableEmpty);
var _yearSelectedData = derived([_selectedData, _selectedYear], function (_ref14) {
  var _ref15 = _slicedToArray(_ref14, 2),
      selectedData = _ref15[0],
      selectedYear = _ref15[1];

  return selectedYear && filter(selectedData, function (_ref16) {
    var year = _ref16.year;
    return year === selectedYear;
  }) || [];
}); // assuming all datapoints in a year have same `region_year_spec`

var _regionsYearSpec = derived(_yearSelectedData, function (yearSelectedData) {
  var _yearSelectedData$;

  return (_yearSelectedData$ = yearSelectedData[0]) === null || _yearSelectedData$ === void 0 ? void 0 : _yearSelectedData$.region_year_spec;
});
var _selectedRegionAtlasIds = derived(_yearSelectedData, function (yearSelectedData) {
  return reduce(yearSelectedData, function (acc, datapoint) {
...
var _selectedRegionIds = derived(_yearSelectedData, function (yearSelectedData) {
  return reduce(yearSelectedData, function (acc, datapoint) {
    if (datapoint.isSelected) {
...
var _valueExtext = derived([_getIndicatorValue, _selectedData], function (_ref17) {
  var _ref18 = _slicedToArray(_ref17, 2),
      getIndicatorValue = _ref18[0],
      selectedData = _ref18[1];

  return extent(selectedData, getIndicatorValue);
});
/* color scale, legend */

var _colorScale = derived([_makeColorScale, _valueExtext], function (_ref19) {
  var _ref20 = _slicedToArray(_ref19, 2),
      makeColorScale = _ref20[0],
      valueExtext = _ref20[1];

  return makeColorScale(valueExtext);
});
var _colorBins = derived([_colorScale, _makeColorBins], function (_ref21) {
  var _ref22 = _slicedToArray(_ref21, 2),
      colorScale = _ref22[0],
      makeColorBins = _ref22[1];

  return makeColorBins(colorScale);
});
